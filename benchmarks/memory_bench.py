...
对比：
  - 静态预分配（Static Allocation）：每条序列开始时就分配 max_tokens 个槽
  - Paged KV Cache（动态分配）    ：按块按需分配，序列结束立即回收
...
def simulate_static_allocation(
    ...
    total_used = 0
    fragmentation_list = []
    prompt_len = 20  # 固定 prompt 长度

    for batch_start in range(0, len(output_lengths), batch_size):
        batch = output_lengths[batch_start: batch_start + batch_size]

        # 每条序列预分配 prompt_len + max_tokens 个槽
        allocated_per_seq = prompt_len + max_tokens
        allocated_this_batch = len(batch) * allocated_per_seq
        used_this_batch = sum(prompt_len + l for l in batch)

        total_allocated += allocated_this_batch
        total_used += used_this_batch

        if allocated_this_batch > 0:
            frag = 1.0 - used_this_batch / allocated_this_batch
            fragmentation_list.append(frag)

    avg_fragmentation = (
        sum(fragmentation_list) / len(fragmentation_list)
        if fragmentation_list else 0
    )
    utilization = total_used / total_allocated if total_allocated > 0 else 0
    # 每条序列固定占用 prompt_len + max_tokens 个槽
    max_concurrent = num_memory_slots // (prompt_len + max_tokens)
...
def simulate_paged_allocation(
    ...
    使用正确的 BlockAllocator（不直接操作内部状态）。
    """
    allocator = BlockAllocator(num_kv_blocks, block_size)

    # 每条序列持有的物理块列表（用于正确释放）
    seq_blocks: Dict[int, List] = {}
    ...
    waiting = list(range(len(output_lengths)))
    ...
    while waiting or running:
        ...
        while waiting and len(running) < max_concurrent_seqs:
            ...
            blocks = [allocator.allocate() for _ in range(needed)]
            seq_blocks[req_id] = blocks
            running[req_id] = 0
        ...
        to_finish = []
        for req_id in list(running.keys()):
            running[req_id] += 1
            total_used_tokens += 1
            ...
            current_blocks = len(seq_blocks[req_id])
            if blocks_needed > current_blocks:
                if allocator.can_allocate(1):
                    new_block = allocator.allocate()
                    seq_blocks[req_id].append(new_block)
        ...
        # 记录利用率
        util_history.append(allocator.utilization)

    avg_util = sum(util_history) / len(util_history) if util_history else 0
    # Paged 利用率 = 实际使用 token 数 / 分配的 token 槽数
    utilization = total_used_tokens / total_allocated_tokens if total_allocated_tokens > 0 else 0
    # 可并发的序列数（每条序列平均需要的块数）
    avg_out_len = sum(output_lengths) / len(output_lengths) if output_lengths else 1
    avg_blocks_per_seq = ((prompt_len + avg_out_len) + block_size - 1) // block_size
    practical_max_concurrent = min(
        max_concurrent_seqs,
        num_kv_blocks // max(1, int(avg_blocks_per_seq)),
    )
    ...
    return {
        ...
        "max_concurrent_seqs": practical_max_concurrent,
...
def run_memory_benchmark(args) -> None:
    ...
    paged_results = simulate_paged_allocation(
        ...
        max_concurrent_seqs=min(64, args.num_kv_blocks // 4),
    )
    ...
    print("=" * 60)
    print(f"  {'Metric':<25} {'Static':>12} {'Paged':>12}")
    print(f"  {'-'*25} {'-'*12} {'-'*12}")
    ...
    metrics = [
        ...
        ("Max Concurrent Seqs", "max_concurrent_seqs"),
    ]
    for label, key in metrics:
        sv = fmt(static_results[key])
        pv = fmt(paged_results[key])
        print(f"  {label:<25} {sv:>12} {pv:>12}")

    util_improvement = (
        paged_results["utilization"] / max(static_results["utilization"], 1e-9)
    )
    concurrency_gain = (
        paged_results["max_concurrent_seqs"] / max(static_results["max_concurrent_seqs"], 1)
    )
...
def _plot_memory_results(static, paged, output_lengths, args):
    ...
    bars2 = ax2.bar(["Static\nAllocation", "Paged\nKV Cache"],
                    frags, color=colors, width=0.5, edgecolor="white")
    ...
    ax3.axvline(mean_len, color="#e74c3c", linestyle="--", linewidth=2,
